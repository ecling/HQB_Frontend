...
(function($, window, document, undefined) {
    ...
    var $window = $(window),
        ...
        defaults = {
            ...
            onloaded:function(){}, // invoke function when data loaded
    ...
    Waterfall.prototype = {
        ...
        _getColumns: function() {
            var options = this.options,
                ...
                cols = Math.floor((containerWidth + gutterWidth) / (colWidth + gutterWidth)),
        ...
        layout: function($content, callback) {
            var options = this.options,
                $items = this.options.isFadeIn ? this._getItems($content).css({
                    opacity: 0
                }).animate({
                    opacity: 1
                }) : this._getItems($content),
        ...
        _requestData: function(callback) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (!data) {
                        options.callbacks.loadingFinished(self.$loading, true);
                    } else {
